...
class MethodWriter implements MethodVisitor {
	...
	public void visitLocalVariable(final String name, final String desc,
		...
		if (signature != null) {
			...
			++localVarTypeCount;
			localVarType.putShort(start.position).putShort(end.position
					- start.position).putShort(cw.newUTF8(name)).putShort(cw.newUTF8(signature)).putShort(index);
		}
		if (localVar == null) {
			localVar = new ByteVector();
		}
		++localVarCount;
		localVar.putShort(start.position).putShort(end.position
				- start.position).putShort(cw.newUTF8(name)).putShort(cw.newUTF8(desc)).putShort(index);
		if (compute != NOTHING) {
			// updates max locals
			final char c = desc.charAt(0);
	...
	public void visitMethodInsn(final int opcode, final String owner,
		...
		final Item i = opcode == Opcodes.INVOKEDYNAMIC ? cw.newNameTypeItem(name, desc)
