...
public class BotToolBar extends JToolBar {
	...
	private class BotButton extends JPanel {
				...
				public void mouseReleased(final MouseEvent e) {
					if (hovered && close) {
						final int idx = getComponentIndex(BotButton.this)
								- TABINDEX;
						listener.actionPerformed(new ActionEvent(this, ActionEvent.ACTION_PERFORMED, Messages.CLOSEBOT
								+ "." + idx));
					} else {
		...
		public void paintComponent(final Graphics g) {
			...
			final int RGB = getComponentIndex(this) == idx ? 255
					: hovered ? 230 : 215;
	...
	private int inputState = Environment.INPUT_KEYBOARD
	...
	private Image getInputImage(final boolean override, final int state) {
		if (override
				|| state == (Environment.INPUT_KEYBOARD | Environment.INPUT_MOUSE)) {
	...
	public void setHome(final boolean home) {
		for (final JButton button : new JButton[] { screenshotButton,
				stopScriptButton, userInputButton, runScriptButton }) {
