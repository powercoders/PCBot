...
public class WebData extends BackgroundScript {
	...
	public boolean activateCondition() {
		...
		return game.isLoggedIn()
				&& (lb == null || !lb.equals(curr_base) || lp == -1 || lp != curr_plane);
	}
	...
	public int loop() {
		try {
			...
			for (int i = 3; i < 102; i++) {
				for (int j = 3; j < 102; j++) {
					final RSTile start = new RSTile(curr_base.getX() + i, curr_base.getY()
							+ j, curr_plane);
					...
					final Short[] theArray = { (short) start.getX(),
							(short) start.getY(), (short) start.getZ() };
					synchronized (lock) {
