...
public class TransportationHandler extends MethodProvider {
	private class Items {
		...
		public final TeleportItem LLETYA_CRYSTAL = new TeleportItem(methods, new RSTile(2328, 3172), new String[] {
				"Activate", "Lletya" }, 6102, 6100, 6101);
	}
	...
	private class Runes {
		...
		public Runes() {
			VARROCK = new TeleportRunes(methods, Magic.SPELL_VARROCK_TELEPORT, Magic.Book.MODERN, new RSTile(3212, 3428, 0), new Rune[] {
					Rune.LAW, Rune.FIRE, Rune.AIR }, new int[] { 1, 1, 3 });
			CAMELOT = new TeleportRunes(methods, Magic.SPELL_CAMELOT_TELEPORT, Magic.Book.MODERN, new RSTile(2757, 3478, 0), new Rune[] {
					Rune.LAW, Rune.AIR }, new int[] { 1, 5 });
	...
	public TransportationHandler(final MethodContext ctx) {
		...
		final Runes runes = new Runes();
		final Objects objects = new Objects();
		final NPCs npcs = new NPCs();
	...
	public boolean canTeleport(final RSTile destination) {
		final Iterator<Teleport> teleportIterator = teleports.listIterator();
		while (teleportIterator.hasNext()) {
			final Teleport teleport = teleportIterator.next();
			if (teleport.meetsPrerequisites()
					&& teleport.isApplicable(methods.players.getMyPlayer().getLocation(), destination)) {
				return true;
			}
		}
		return false;
