...
public class TeleportRunes extends Teleport {
    ...
    public TeleportRunes(final MethodContext ctx, final int spell,
	    ...
	    final Rune[] runes, final int[] count) throws Exception {
	...
	if (rune.isElemental()) {
	    String wepName = methods.equipment.getItem(Equipment.WEAPON) != null ? methods.equipment
		    .getItem(Equipment.WEAPON).getName() : "";
	    if (rune == Rune.WATER) {
		String shieldName = methods.equipment.getItem(Equipment.SHIELD) != null ? methods.equipment
			.getItem(Equipment.SHIELD).getName() : "";
		if (shieldName != null
