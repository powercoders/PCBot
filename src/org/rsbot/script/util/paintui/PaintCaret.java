...
public class PaintCaret {
	...
	public Line2D getCaretPosition(int dotPos) {
		...
		Rectangle2D strBounds = field.getCurrentStyle().font.getStringBounds(off, field.frc);
		return new Line2D.Double(field.getAbsoluteX() + 3 + (dotPos >= 0 ? (int) strBounds.getWidth() : 0), field.getAbsoluteY() + field.getHeight() - 2, field.getAbsoluteX() + 3 + (dotPos >= 0 ? (int) strBounds.getWidth() : 0), field.getAbsoluteY() + 2);
	}
	...
	public void doKeyEvent(KeyEvent e) {
		if (e.getKeyCode() == KeyEvent.VK_LEFT && dot >= 0) {
		...
		} else if (e.getKeyCode() == KeyEvent.VK_RIGHT && dot < field.getText().length()) {
			if (e.isShiftDown()) {
				moveDot(dot + 1);
			} else {
				setDot(dot + 1);
			}
		} else if (e.getKeyCode() == KeyEvent.VK_UP || e.getKeyCode() == KeyEvent.VK_END || e.getKeyCode() == KeyEvent.VK_PAGE_DOWN) {
			if (e.isShiftDown()) {
				moveDot(field.getText().length());
			} else {
				setDot(field.getText().length());
			}
		} else if (e.getKeyCode() == KeyEvent.VK_DOWN || e.getKeyCode() == KeyEvent.VK_HOME || e.getKeyCode() == KeyEvent.VK_PAGE_UP) {
			if (e.isShiftDown()) {
				moveDot(-1);
			} else {
				setDot(-1);
			}
		} else if (e.getKeyChar() == '\b' && (Math.min(dot, mark) > 0 || dot != mark)) {
			if (dot == mark) {
				String b4 = field.getText().substring(0, Math.min(dot, mark) - 1);
				String after = "";
				...
				String b4 = Math.min(dot, mark) > 0 ? field.getText().substring(0, Math.min(dot, mark)) : "";
				String after = "";
				if (Math.max(dot, mark) >= 0) {
					after = field.getText().substring(Math.max(dot, mark));
				}
		...
		} else if (e.getKeyCode() == KeyEvent.VK_DELETE && ((dot < field.getText().length() && dot >= 0) || dot != mark)) {
			if (dot == mark) {
			...
			Rectangle2D strBounds = field.getCurrentStyle().font.getStringBounds(newText, field.frc);
