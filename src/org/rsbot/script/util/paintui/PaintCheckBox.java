...
public class PaintCheckBox extends PaintComponent {
	...
	public void paint(Graphics render) {
		...
		g.fill3DRect(boxRect.x, boxRect.y, boxRect.width, boxRect.height, !checked);
		if (getCurrentStyle().fgColor != null && getCurrentStyle().font != null) {
			...
			g.drawString(getDisplay(), boxRect.x + boxRect.width + 3, boxRect.y + boxRect.height - 2);
		}
		if (getCurrentStyle().border != null) {
			g.setColor(getCurrentStyle().border);
			g.draw3DRect(boxRect.x, boxRect.y, boxRect.width, boxRect.height, !checked);
		}
		g.dispose();
	}

	private String getDisplay() {
		return label.replace("[B]", "").replace("[I]", "").replace("[i]", "").replace("[b]", "");
	}
