...
public class PaintUtil {
	...
	public void drawPaint(final int skill, final int startXP, final int amount,
		...
		if (!isHidden) {
			...
			final long ttl = ctx.skills.getTimeTillNextLevel(skill, startXP,
			...
			if (img == null) {
				img = (BufferedImage) getImage(String.valueOf(skill) + ".png",
						true,
	...
	public void drawProgressBar(final int skill, final int x, final int y,
		...
		final String progress = ctx.skills.getPercentToNextLevel(skill)
				+ "% to " + (ctx.skills.getCurrentLevel(skill) + 1) + " "
			...
			for (RSItem i : curItem) {
				if (i.getID() != -1) {
					if (i != null) {
						...
						g2.draw(bounds);
						g2.drawString(name, i.getComponent().getAbsoluteX(), i
	...
	public void drawWebMM(final int alpha) {
		...
		for (final TileFlags t : tiles) {
			if (t != null) {
				...
				} else if (!t.isWater() && !t.isWalkable()
						&& !t.isQuestionable()) {
