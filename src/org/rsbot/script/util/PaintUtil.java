...
public class PaintUtil {
	...
	public void drawWebOnScreen(int alpha) {
		...
		for (TileFlags t : tiles) {
			if (t != null) {
				if (t.isWalkable()) {
					drawTileOnScreen(t.getTile(), Color.GREEN, alpha);
				} else if (t.isQuestionable()) {
					drawTileOnScreen(t.getTile(), Color.YELLOW, alpha);
				} else if (t.isWater()) {
					drawTileOnScreen(t.getTile(), Color.CYAN, alpha);
				}
			}
		}
	}
	*/
	...
	public void drawWebMM(int alpha) {
		...
		for (TileFlags t : tiles) {
			if (t != null) {
				if (t.isWalkable()) {
					drawTileMM(t.getTile(), Color.GREEN, alpha);
				} else if (t.isQuestionable()) {
					drawTileMM(t.getTile(), Color.YELLOW, alpha);
				} else if (t.isWater()) {
					drawTileMM(t.getTile(), Color.CYAN, alpha);
				} else if (!t.isWater() && !t.isWalkable() && !t.isQuestionable()) {
					drawTileMM(t.getTile(), Color.RED, alpha);
				}
			}
		}
	}
