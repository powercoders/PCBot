...
/**
 * Update by Iscream (Apr 24,2010) Update by Iscream (Apr 15,2010) Update by
 * Arbiter (Sep 25,2010)
 * 
...
public class BeehiveSolver extends Random {
	...
	private static final int[] BEEHIVE_ARRAYS = { ID_TOP, ID_MIDUP, ID_MIDDOWN,
			ID_DOWN };
	private static final String[] MODEL_NAMES = { "Top", "Middle Up",
			"Middle Down", "Down" };
	boolean solved;
	...
	public boolean activateCondition() {
		...
		if (npcs.getNearest(BEEHIVE_KEEPER_ID) != null
				&& objects.getNearest(16168) != null) {
			solved = false;
			return true;
		}

		/*
		 * BeehiveKeeper = npcs.getNearest(BEEHIVE_KEEPER_ID); if
		 * ((BeehiveKeeper != null) || getBeehiveInterface().isValid()) {
		 * sleep(random(1000, 1500)); BeehiveKeeper =
		 * npcs.getNearest(BEEHIVE_KEEPER_ID); if ((BeehiveKeeper != null) ||
		 * getBeehiveInterface().isValid()) { solved = false; sleep(random(1000,
		 * 1500)); return true; } }
		 */
		return false;
	}

	public boolean dragInterfaces(final RSComponent child1,
			final RSComponent child2) {
	...
	public int loop() {
		...
		if (getBeehiveInterface().isValid()) {
			...
			if (solved
					&& interfaces.getComponent(INTERFACE_BEEHIVE_WINDOW, BUILD_BEEHIVE).doClick()) {
	...
	public int returnIdAtSlot(final int slot) {
		...
		for (int i = 0; i < BEEHIVE_ARRAYS.length; i++) {
			if (Model_ID == BEEHIVE_ARRAYS[i]) {
				log.info("Slot " + slot + " contains section: "
						+ MODEL_NAMES[i]);
				return Model_ID;
			}
		}
