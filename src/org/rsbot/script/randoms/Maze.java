...
public class Maze extends Random {
	...
	public boolean atDoorTiles(final RSTile a, final RSTile b) {
		...
		if (!calc.tileOnScreen(a) || !calc.tileOnScreen(b)
				|| calc.distanceTo(a) > random(4, 7)) {
			if (calc.tileOnMap(a)) {
			...
			try {
				...
				final Point location = new Point((int) (xTotal / thePoints.length), (int) (yTotal / thePoints.length)
						- random(0, 40));
				if (location.x == -1 || location.y == -1) {
					return false;
				}
				if (Math.sqrt(Math.pow((mouse.getLocation().getX() - location.getX()), 2)
						+ Math.pow((mouse.getLocation().getY() - location.getY()), 2)) < random(20, 30)) {
	...
	public void getNewWall() {
		for (final Door door : paths) {
			if (door.doorID == doorIndex) {
				...
				log("walking.walkTo: " + walkToTile.getX() + ", "
						+ walkToTile.getY());
				log("Door index: " + doorIndex + " | Door direction: "
						+ doorDir);
			}
		}
	}
