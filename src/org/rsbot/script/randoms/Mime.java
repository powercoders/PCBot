...
public class Mime extends Random {
	...
	private Stage getStage() {
		if (interfaces.canContinue()
				&& getMyPlayer().getLocation().equals(new RSTile(2008, 4764))) {
			return Stage.click;
		} else if (mime == null) {
			return Stage.findMime;
		} else if ((interfaces.get(372).getComponent(2).getText().contains("Watch") || interfaces.get(372).getComponent(3).getText().contains("Watch"))
	...
	public int loop() {
		...
		case findMime:
			if ((mime = npcs.getNearest(1056)) == null
			...
			if (animation != -1 && animation != 858) {
				...
				default:
					log.info("Unknown Animation: " + animation
							+ " Please inform a developer at RSBot.org!");
