...
public class LoginBot extends Random {
	...
	private static final int INTERFACE_WELCOME_SCREEN_NOT_LOGGED_OUT_TEXT = 224;
	private static final int INTERFACE_WELCOME_SCREEN_NOT_LOGGED_OUT_BUTTON = 231;
	...
	public int loop() {
		...
		if (game.getClientState() != INDEX_LOGGED_OUT) {
			...
			if (game.getClientState() == INDEX_LOBBY) {
				...
				if (interfaces.get(INTERFACE_WELCOME_SCREEN).getComponent(
						INTERFACE_WELCOME_SCREEN_NOT_LOGGED_OUT_TEXT).getText().toLowerCase().trim().contains("has not logged out")) {
					interfaces.get(INTERFACE_WELCOME_SCREEN).getComponent(
							INTERFACE_WELCOME_SCREEN_NOT_LOGGED_OUT_BUTTON).doClick();
				}
			}
			return -1;
		}
