 ...
 */
@ScriptManifest(authors = { "Arbiter", "PwnZ", "Megaalgos", "Taha", "Fred",
		"Poxer", "Iscream" }, name = "Exam", version = 2.1)
public class Exam extends Random {
	public class NextObjectQuestion {
		...
		public boolean clickAnswer() {
			...
			for (int i = 10; i <= 13; i++) {
				if (arrayContains(Answers, interfaces.get(nextObjectInterface).getComponent(i).getComponentID())) {
		...
		public int[] returnAnswer() {
			...
			if (secondcard == 0) {
				...
				log.severe("The Missing Object ID is :" + Integer.toString(Two));
			}
			if (thirdcard == 0) {
	...
	public class SimilarObjectQuestion {
		...
		public boolean activateCondition() {
			...
			if (interfaces.getComponent(relatedCardsInterface, 25).getText().toLowerCase().contains(question)) {
	...
	public int loop() {
		...
		if (!interfaces.get(nextObjectInterface).isValid()
				...
				&& !interfaces.canContinue() && door == null) {
