...
public class InputManager {
	...
	public void holdKey(final int keyCode, final int ms) {
		...
		if (ms > 500) {
			...
			for (int i = 37; i < ms2; i += random(20, 40)) {
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED, System.currentTimeMillis()
						+ i + 500, 0, keyCode, (char) keyCode);
				getClient().getKeyboard()._keyPressed(ke);
			}
		}
		final int delay2 = ms + random(-30, 30);
		ke = new KeyEvent(getTarget(), KeyEvent.KEY_RELEASED, System.currentTimeMillis()
				+ delay2, 0, keyCode, (char) keyCode);
	...
	public void moveMouse(final int speed, final int x, final int y,
		...
		windMouse(speed, thisX, thisY, random(x, x + randomX), random(y, y
				+ randomY));
	}
	...
	private void pressMouse(final int x, final int y, final boolean left) {
		if (getClient().getMouse().isPressed()
				|| !getClient().getMouse().isPresent()) {
			return;
		}
		final MouseEvent me = new MouseEvent(getTarget(), MouseEvent.MOUSE_PRESSED, System.currentTimeMillis(), 0, x, y, 1, false, left ? MouseEvent.BUTTON1
				: MouseEvent.BUTTON3);
		getClient().getMouse().sendEvent(me);
	}
	...
	private void releaseMouse(final int x, final int y, final boolean leftClick) {
		...
		MouseEvent me = new MouseEvent(getTarget(), MouseEvent.MOUSE_RELEASED, System.currentTimeMillis(), 0, x, y, 1, false, leftClick ? MouseEvent.BUTTON1
				: MouseEvent.BUTTON3);
		getClient().getMouse().sendEvent(me);

		if ((dragLength & 0xFF) <= 3) {
			me = new MouseEvent(getTarget(), MouseEvent.MOUSE_CLICKED, System.currentTimeMillis(), 0, x, y, 1, false, leftClick ? MouseEvent.BUTTON1
					: MouseEvent.BUTTON3);
			getClient().getMouse().sendEvent(me);
		}
		// reset
	...
	private void sendKey(final char ch, final int delay) {
		...
		if (code == KeyEvent.VK_LEFT || code == KeyEvent.VK_UP
			...
			ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED, System.currentTimeMillis()
					+ delay, 0, code, getKeyChar(ch), KeyEvent.KEY_LOCATION_STANDARD);
			getClient().getKeyboard()._keyPressed(ke);
			final int delay2 = random(50, 120) + random(0, 100);
			ke = new KeyEvent(getTarget(), KeyEvent.KEY_RELEASED, System.currentTimeMillis()
					+ delay2, 0, code, getKeyChar(ch), KeyEvent.KEY_LOCATION_STANDARD);
			getClient().getKeyboard()._keyReleased(ke);
		} else {
			if (!shift) {
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED, System.currentTimeMillis()
				...
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED, System.currentTimeMillis()
						+ s1, InputEvent.SHIFT_DOWN_MASK, KeyEvent.VK_SHIFT, (char) KeyEvent.VK_UNDEFINED, KeyEvent.KEY_LOCATION_LEFT);
				getClient().getKeyboard()._keyPressed(ke);

				// Event Pressed for char to send
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_PRESSED, System.currentTimeMillis()
						+ delay, InputEvent.SHIFT_DOWN_MASK, code, getKeyChar(ch), KeyEvent.KEY_LOCATION_STANDARD);
				getClient().getKeyboard()._keyPressed(ke);
				...
				final int delay2 = random(50, 120) + random(0, 100);
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_RELEASED, System.currentTimeMillis()
						+ delay2, InputEvent.SHIFT_DOWN_MASK, code, getKeyChar(ch), KeyEvent.KEY_LOCATION_STANDARD);
				getClient().getKeyboard()._keyReleased(ke);

				// Event Released for shift key
				final int s2 = random(25, 60) + random(0, 50);
				ke = new KeyEvent(getTarget(), KeyEvent.KEY_RELEASED, System.currentTimeMillis()
						+ s2, InputEvent.SHIFT_DOWN_MASK, KeyEvent.VK_SHIFT, (char) KeyEvent.VK_UNDEFINED, KeyEvent.KEY_LOCATION_LEFT);
				getClient().getKeyboard()._keyReleased(ke);
			}
		}
	}
