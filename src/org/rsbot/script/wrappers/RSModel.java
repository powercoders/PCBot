...
public abstract class RSModel extends MethodProvider {
	...
	private Point getPointInRange(final int start, final int end) {
		...
		for (int i = start; i < end; ++i) {
			final Point one = methods.calc.worldToScreen(locX
					+ xPoints[indices1[i]], locY + zPoints[indices1[i]], height
					+ yPoints[indices1[i]]);
			int x = -1, y = -1;
			if (one.x >= 0) {
				x = one.x;
			...
			final Point two = methods.calc.worldToScreen(locX
					+ xPoints[indices2[i]], locY + zPoints[indices2[i]], height
					+ yPoints[indices2[i]]);
			if (two.x >= 0) {
				if (x >= 0) {
					x = (x + two.x) / 2;
	...
	public Polygon[] getTriangles() {
		...
		for (int i = 0; i < len; ++i) {
			...
			if (one.x >= 0 && two.x >= 0 && three.x >= 0) {
				polygons.add(new Polygon(new int[] { one.x, two.x, three.x }, new int[] {
