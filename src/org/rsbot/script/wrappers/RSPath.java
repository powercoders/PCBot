...
public abstract class RSPath extends MethodProvider {
	...
	/**
	 * Checks whether or not this path can be traversed by the player. This will
	 * be the case provided that the player near to one of its vertices, but not
	 * already standing on the end vertex.
	 * 
	 * @return <tt>true</tt> if the player can walk along this path; otherwise
	 *         <tt>false</tt>.
	 */
	public abstract boolean isValid();

	/**
	 * Takes a step along this path if appropriate. Specifies only
	 * TraversalOption.SPACE_ACTIONS.
	 * 
	 * @return <tt>true</tt> if this path is currently valid for the player;
	...
	/**
	 * Takes a step along this path if appropriate. If the path cannot be
	 * traversed due to the player being too far from its vertices or already at
	 * the end vertex, <tt>false</tt> will be returned. In all other cases,
	 * <tt>true</tt> will be returned, but an action will not necessarily be
	 * performed (based on the given options).
	 * 
	 * @param options
