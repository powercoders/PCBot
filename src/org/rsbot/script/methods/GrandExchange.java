...
public class GrandExchange extends MethodProvider {
	...
	public static class GEItem {
		...
		GEItem(final String name, final String examine, final int id,
				final double[] values) {
			this.name = name;
		...
		for (int i = 1; i <= 6; i++) {
			if (isOpen()) {
				if (checkSlotIsEmpty(i)) {
					...
					final String s = methods.interfaces.getComponent(INTERFACE_GRAND_EXCHANGE_WINDOW, slotComponent).getComponent(18).getText();
					if (s.equals(name)) {
						return i;
					}
				}
	...
	public boolean isOpen() {
		return methods.interfaces.get(INTERFACE_GRAND_EXCHANGE_WINDOW).isValid();
	}
	...
	public GEItem lookup(final int itemID) {
		try {
			final URL url = new URL(GrandExchange.HOST + GrandExchange.GET
					+ itemID);
