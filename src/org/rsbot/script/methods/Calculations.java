...
public class Calculations extends MethodProvider {
	...
	public int angleToTile(final RSTile t) {
		...
		final int angle = (int) Math.toDegrees(Math.atan2(t.getY() - me.getY(), t.getX()
				- me.getX()));
	 ...
	 * @return <tt>true</tt> if a path can be made to the specified tile;
	...
	public int distanceTo(final RSTile t) {
		return t == null ? -1
				: (int) distanceBetween(methods.players.getMyPlayer().getLocation(), t);
	}
	...
	public Point groundToScreen(final int x, final int y, final int height) {
		if (methods.client.getGroundByteArray() == null
				|| methods.client.getTileData() == null || x < 512 || y < 512
				|| x > 52224 || y > 52224) {
	 ...
	 *         should be accepted.
	 */
	public int pathLengthBetween(final RSTile start, final RSTile dest,
	...
	public Point tileToScreen(final RSTile tile, final double dX,
		...
		return groundToScreen((int) ((tile.getX() - methods.client.getBaseX() + dX) * 512), (int) ((tile.getY()
				- methods.client.getBaseY() + dY) * 512), height);
	}
	...
	public Point worldToScreen(final int x, final int y, final int z) {
		...
		final float _z = renderData.zOff
				+ (int) (renderData.zX * x + renderData.zY * z + renderData.zZ
						* y);
		if (_z >= render.zNear && _z <= render.zFar) {
			final int _x = (int) (render.xMultiplier
					* ((int) renderData.xOff + (int) (renderData.xX * x
							+ renderData.xY * z + renderData.xZ * y)) / _z);
			final int _y = (int) (render.yMultiplier
