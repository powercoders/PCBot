...
public class Combat extends MethodProvider {
	...
	public int getHealth() {
		return getLifePoints() * 10
				/ methods.skills.getRealLevel(Skills.CONSTITUTION);
	}
	...
	public int getWildernessLevel() {
		return methods.interfaces.get(381).getComponent(2).isValid() ? Integer.parseInt(methods.interfaces.get(381).getComponent(2).getText().replace("Level: ", "").trim())
				: 0;
	}
		...
		if (fightMode != getFightMode()) {
			...
			if (fightMode == 0) {
				...
				return methods.interfaces.getComponent(884, 12).doClick();
					...
					|| fightMode == 3
					&& methods.interfaces.getComponent(884, 14).getActions() == null) {
