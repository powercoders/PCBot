...
public class Summoning extends MethodProvider {
	...
	public boolean doDismiss() {
		final RSComponent confirmInterface = methods.interfaces.getComponent(228, 2);
		if (confirmInterface.isValid()) {
			return confirmInterface.doClick();
		} else {
			final RSComponent dismissInterface = methods.interfaces.getComponent(INTERFACE_TAB_SUMMONING, INTERFACE_TAB_SUMMONING_CHILD);
			if (dismissInterface != null
					&& dismissInterface.doAction("Dismiss")) {
