...
public class Trade extends MethodProvider {
	...
	public boolean acceptTrade() {
		if (inTradeMain()) {
			...
			return inTradeSecond()
					&& methods.interfaces.get(INTERFACE_TRADE_SECOND).getComponent(INTERFACE_TRADE_SECOND_ACCEPT).doAction("Accept");
		}
	}
	...
	public boolean declineTrade() {
		if (inTradeMain()) {
			...
			return inTradeSecond()
					&& methods.interfaces.get(INTERFACE_TRADE_SECOND).getComponent(INTERFACE_TRADE_SECOND_DECLINE).doAction("Decline");
		}
	}
	...
	public RSItem[] getItemsOffered() {
		final List<RSItem> items = new ArrayList<RSItem>();
		for (int i = 0; i < 28; i++) {
			final RSComponent component = methods.interfaces.get(INTERFACE_TRADE_MAIN).getComponent(INTERFACE_TRADE_MAIN_THEIR).getComponent(i);
	...
	public boolean tradePlayer(final RSPlayer targetPlayer, final int tradeWait) {
		if (!inTrade()) {
			if (targetPlayer != null) {
				return targetPlayer.doAction("Trade with", targetPlayer.getName())
