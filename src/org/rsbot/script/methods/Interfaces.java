...
public class Interfaces extends MethodProvider {
	...
	public boolean clickComponent(final RSComponent c, final String action) {
		...
		methods.mouse.move(random(minX, minX + width), random(minY, minY
				+ height));
	 ...
	 * @return true when scrolled successfully
	 */
	public boolean scrollTo(final RSComponent component,
			final RSComponent scrollBar) {
		// Check arguments
		...
		int pos = (int) ((float) scrollBarArea.getRealHeight() / contentHeight * (component.getRelativeY() + random(-areaHeight / 2, areaHeight
		...
		methods.mouse.click(scrollBarArea.getAbsoluteX()
				+ random(0, scrollBarArea.getRealWidth()), scrollBarArea.getAbsoluteY()
				+ pos, true);
	 ...
	 */
	public boolean waitFor(final RSInterface iface, final boolean valid,
			final int timer) {
