...
public class Bank extends MethodProvider {
	...
	public static final Filter<RSObject> OBJECT_DEPOSIT_BOX = new Filter<RSObject>() {
		...
		public boolean accept(final RSObject rsObject) {
			final String name = rsObject != null ? rsObject.getName() : null;
			for (final String bankName : depositBoxNames) {
				if (name.equalsIgnoreCase(bankName)) {
					return true;
				}
			}
	...
	public static final Filter<RSNPC> NPC_BANKERS = new Filter<RSNPC>() {
		...
		public boolean accept(final RSNPC rsNPC) {
			...
			for (final String bankName : bankerNames) {
				if (name.equalsIgnoreCase(bankName)) {
					return true;
				}
			}
	...
	public boolean deposit(final int itemID, final int number) {
		if (isOpen() || isDepositOpen()) {
			...
			int itemCount = 0;
			final int invCount = isOpen() ? methods.inventory.getCount(true)
					: getBoxCount();
			if (!isOpen()) {
				boolean match = false;
				for (int i = 0; i < 28; i++) {
			...
			sleep(300);
			final int cInvCount = isOpen() ? methods.inventory.getCount(true)
					: getBoxCount();
	...
	public boolean depositAll() {
		...
		return isDepositOpen()
				&& methods.interfaces.getComponent(INTERFACE_DEPOSIT_BOX, INTERFACE_DEPOSIT_BUTTON_DEPOSIT_CARRIED_ITEMS).doClick();
	}
	...
	public boolean depositAllEquipped() {
		...
		return isDepositOpen()
				&& methods.interfaces.getComponent(INTERFACE_DEPOSIT_BOX, INTERFACE_DEPOSIT_BUTTON_DEPOSIT_WORN_ITEMS).doClick();
	}
	...
	public boolean depositAllFamiliar() {
		...
		return isDepositOpen()
				&& methods.interfaces.getComponent(INTERFACE_DEPOSIT_BOX, INTERFACE_DEPOSIT_BUTTON_DEPOSIT_BEAST_INVENTORY).doClick();
	}
	...
	public boolean open() {
		...
		try {
			...
			final RSNPC banker = methods.npcs.getNearest(NPC_BANKERS);
			/*
			 * Find closest one, others are set to null. Remember distance and
			 * tile.
			 */
			...
			if (lowestDist < 5 && methods.calc.tileOnMap(tile)
					&& methods.calc.canReach(tile, true)) {
	...
	public boolean openEquipment() {
		return getInterface().getComponent(INTERFACE_BANK_BUTTON_OPEN_EQUIP).isValid()
				&& getInterface().getComponent(INTERFACE_BANK_BUTTON_OPEN_EQUIP).doClick();
	}
	...
	public boolean openTab(final int tabNumber) {
		return isOpen()
				&& methods.interfaces.getComponent(INTERFACE_BANK, INTERFACE_BANK_TAB[tabNumber - 1]).doClick();
	}
