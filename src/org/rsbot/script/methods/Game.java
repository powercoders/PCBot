...
public class Game extends MethodProvider {
	...
	public enum ChatButton {
		ALL(2, -1, 31), GAME(3, 30, 28, ChatMode.ALL, ChatMode.FILTER), PUBLIC(
				4, 27, 25, ChatMode.ON, ChatMode.FRIENDS, ChatMode.OFF,
				ChatMode.HIDE), PRIVATE(5, 24, 22, ChatMode.ON,
				ChatMode.FRIENDS, ChatMode.OFF), FRIENDS(7, 35, 33,
				ChatMode.ON, ChatMode.FRIENDS, ChatMode.OFF), CLAN(6, 21, 19,
				ChatMode.ON, ChatMode.FRIENDS, ChatMode.OFF), TRADE(8, 18, 16,
				ChatMode.ON, ChatMode.FRIENDS, ChatMode.OFF), ASSIST(9, 15, 13,
				ChatMode.ON, ChatMode.FRIENDS, ChatMode.OFF);
		...
		ChatButton(final int component, final int textComponent,
				final int selectComponent, final ChatMode... options) {
		...
		public boolean hasMode(final ChatMode mode) {
			...
			for (final ChatMode option : options) {
				if (mode == option) {
					return true;
				}
			}
	 ...
	 */
	public enum Tab {
		NONE(-1, "None", 0, -1), ATTACK(0, "Combat Styles", KeyEvent.VK_F5, 884), TASK(
				1, "Task System", 0, 1056), STATS(2, "Stats", 0,
				Skills.INTERFACE_TAB_STATS), QUESTS(3, "Quest Journals", 0,
				Quests.QUESTS), INVENTORY(4, "Inventory", KeyEvent.VK_F1,
				Inventory.INTERFACE_INVENTORY), EQUIPMENT(5, "Worn Equipment",
				KeyEvent.VK_F2, Equipment.INTERFACE_EQUIPMENT), PRAYER(6,
				"Prayer List", KeyEvent.VK_F3, Prayer.INTERFACE_PRAYER), MAGIC(
				7, "Magic Spellbook", KeyEvent.VK_F4, 192), FRIENDS(9,
				"Friends List", 0, 550), FRIENDS_CHAT(10, "Friends Chat", 0,
				FriendChat.INTERFACE_FRIEND_CHAT), CLAN_CHAT(11, "Clan Chat",
				0, ClanChat.INTERFACE_CLAN_CHAT), OPTIONS(12, "Options", 0, 261), EMOTES(
				13, "Emotes", 0, 464), MUSIC(14, "Music Player", 0, 187), NOTES(
				15, "Notes", 0, 34), LOGOUT(16, "Exit", 0,
				Game.INTERFACE_LOGOUT);
		...
		Tab(final int index, final String description, final int functionKey,
				final int inter) {
	...
	public static final int[] INTERFACE_TALKS = { 211, 241, 251, 101, 242, 102,
			161, 249, 243, 64, 65, 244, 255, 249, 230, 372, 421 };
	 ...
	 * 
	 * @return <tt>true</tt> if the setting was clicked; otherwise
	 *         <tt>false</tt>.
	 */
	...
	/**
	 * Fetch the chat button at the provided index for deprecated methods. For
	 * internal use only.
	 */
	private ChatButton getButton(final int idx) {
		for (final ChatButton b : ChatButton.values()) {
			if (b.idx() == idx) {
				return b;
			}
		}
	 ...
	 * 
	 * @return <tt>RSComponent</tt> of the current valid talk interface;
	 *         otherwise null.
	 * @see #INTERFACE_TALKS
	 */
	...
	public boolean isWelcomeScreen() {
		return methods.interfaces.getComponent(INTERFACE_WELCOME_SCREEN, INTERFACE_WELCOME_SCREEN_CHILD).getAbsoluteY() > 2;
	}
	...
	public boolean logout(final boolean lobby) {
		...
		if (getTab() != Tab.LOGOUT) {
			...
			final RSComponent exitComponent = methods.interfaces.getComponent(idx, isFixed() ? 181
					: 173);
		...
		final RSComponent exitToComponent = methods.interfaces.getComponent(INTERFACE_LOGOUT, lobby ? 2
				: 13);
	 ...
	 *            One of ChatButton
	 * @param left
	 *            true to left click, false for right click.
	 ...
	 *            to switch.
	 * @param functionKey
	 *            Use a function key (if available) for fast switching.
