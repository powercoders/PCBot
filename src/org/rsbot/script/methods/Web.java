...
public class Web extends MethodProvider {
	 ...
	 */
	public static boolean Flag(final int x, final int y, final int z,
			final int key) {
	...
	public static boolean Flag(final RSTile tile, final int key) {
		final Short[] tileData = { (short) tile.getX(), (short) tile.getY(),
				(short) tile.getZ() };
		if (Web.rs_map.containsKey(tileData)) {
			final int theTile = Web.rs_map.get(tileData);
			return (theTile & key) != 0;
	 ...
	 * @return <tt>true</tt> if the tile contains flags.
	 */
	public static boolean Flag(final short x, final short y, final short z,
			final int key) {
	...
	private static List<Node> Successors(final Node t) {
		...
		if (!Flag(here, TileData.Key.W_S)
				&& !Flag(here.getX(), here.getY() - 1, here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER)) {
			tiles.add(new Node(x, y - 1, here.getZ()));
		}
		if (!Flag(here, TileData.Key.W_W)
				&& !Flag(here.getX() - 1, here.getY(), here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER)) {
		...
		if (!Flag(here, TileData.Key.W_SW | TileData.Key.W_S | TileData.Key.W_W)
				&& !Flag(here.getX() - 1, here.getY() - 1, here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER)
				&& !Flag(here.getX(), here.getY() - 1, here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER | TileData.Key.W_W)
				&& !Flag(here.getX() - 1, here.getY(), here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER | TileData.Key.W_S)) {
		...
		if (!Flag(here, TileData.Key.W_NW | TileData.Key.W_N | TileData.Key.W_W)
				...
				&& !Flag(here.getX(), here.getY() + 1, here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER | TileData.Key.W_W)
				&& !Flag(here.getX() - 1, here.getY(), here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER | TileData.Key.W_N)) {
			tiles.add(new Node(x - 1, y + 1, here.getZ()));
		}
		if (!Flag(here, TileData.Key.W_SE | TileData.Key.W_S | TileData.Key.W_E)
				&& !Flag(here.getX() + 1, here.getY() - 1, here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER)
				&& !Flag(here.getX(), here.getY() - 1, here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER | TileData.Key.W_E)
				&& !Flag(here.getX() + 1, here.getY(), here.getZ(), TileData.Key.BLOCKED
						| TileData.Key.WATER | TileData.Key.W_S)) {
			tiles.add(new Node(x + 1, y - 1, here.getZ()));
		}
	 ...
	 */
	private Route[] generateRoutes(final RSTile start, final RSTile end,
		...
		if (start.getZ() == end.getZ()) {
			...
			final LinkedList<Route> finalRouting = new LinkedList<Route>();
		...
		final PlaneHandler planeHandler = new PlaneHandler(methods);
		final PlaneTraverse[] traverses = planeHandler.get(methods.game.getPlane());
		for (final PlaneTraverse traverse : traverses) {
			if (traverse.destPlane() == end.getZ()) {// TODO more complex
														// method--prevent
														// infinite loops once
														// made.
	...
	public RSTile getNearestBank(final RSTile tile) {
		...
		final RSTile[] BANKS = { new RSTile(3093, 3243, 0),
				new RSTile(3209, 3219, 2), new RSTile(3270, 3167, 0),
				new RSTile(3253, 3421, 0), new RSTile(3188, 3437, 0),
				new RSTile(3094, 3491, 0), new RSTile(3097, 3496, 0),
				new RSTile(2946, 3369, 0), new RSTile(3012, 3356, 0) };
		for (final RSTile bank : BANKS) {
			final double cdist = methods.calc.distanceBetween(tile, bank);
	 ...
	 * @return The web constructed. <code>null</code> if it cannot be done.
	 */
	public RSWeb getWeb(final RSTile start, final RSTile end) {
		final Route[] routes = generateRoutes(start, end, null);
		return new RSWeb(routes, start, end);
	}

	public Route planeRoute(final RSTile start, final RSTile end,
			final PlaneTraverse transfer) {
		if (transfer != null) {
		...
		final RSTile[] path = generateTilePath(start, end); // TODO add
															// teleports object
															// etc
		if (path == null) {
			return null;
