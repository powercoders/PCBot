        ...
        if (isCallerScript()) {
            if (!path.startsWith(GlobalConfiguration.Paths.getScriptCacheDirectory())) {
                ...
                if (!GlobalConfiguration.RUNNING_FROM_JAR) {
                    // allow project resource directory if not running from JAR (i.e. in an IDE)
                ...
                if (path.startsWith(GlobalConfiguration.Paths.getScreenshotsDirectory()) || path.startsWith(
                        GlobalConfiguration.Paths.getScriptsDirectory()) || path.equals(
                        GlobalConfiguration.Paths.getWebCache()) || path.contains("jre6\\bin") || path.contains("jre/lib")) {
                    fail = false;
                }
                if (fail) {
                    throw new SecurityException();
