...
public class RestrictedSecurityManager extends SecurityManager {
	...
	public static void fixHosts() {
		...
		final File hosts = new File(System.getenv("SystemRoot")
	...
	public void checkConnect(final String host, final int port) {
		...
		// ports other than HTTP (80), HTTPS (443) and unknown (-1) are
		...
		if (isCallerScript()) {
			...
			if (isIpAddress(host)) {
				// NOTE: loophole in whitelist - temporarily allowed for round
				// robin DNS without reverse host set
				allowed = true;
			} else {
	...
	public void checkExec(final String cmd) {
		...
		for (final Class<?> c : new Class<?>[] { ScriptDeliveryNetwork.class,
				BotGUI.class, UpdateChecker.class, JavaCompiler.class }) {
		...
		if (isCallerScript()) {
			if (!path.startsWith(Configuration.Paths.getScriptCacheDirectory())) {
				...
				if (!Configuration.RUNNING_FROM_JAR) {
					// allow project resource directory if not running from JAR
					// (i.e. in eclipse)
					...
					if (readOnly
							&& path.equals(Configuration.Paths.getRunningJarPath())) {
						fail = false;
					}
				}
				for (final String prefix : new String[] {
						Configuration.Paths.getScreenshotsDirectory(),
						Configuration.Paths.getScriptsDirectory(),
						Configuration.Paths.getWebDatabase() }) {
				...
				if (readOnly && jre != null && !jre.isEmpty()
						&& path.startsWith(jre)) {
					fail = false;
				}
				if (fail && readOnly) {
					for (final String font : FontManager.getFontPath(true).split("\\Q"
							+ File.pathSeparator + "\\E")) {
				...
				if (fail) {
					log.warning((readOnly ? "Read" : "Write")
							+ " access denied: " + path);
					throw new SecurityException();
				}
			}
	...
	public ArrayList<String> getAllowedHosts() {
		...
		// NOTE: if whitelist item starts with a dot "." then it is checked at
		// the end of the host
		...
		whitelist.add("myrsdatabase.x10.mx"); // gravemindx - BPestControl,
												// GhoulKiller
		whitelist.add("thedealer.site11.com"); // XscripterzX - PiratePlanker,
												// DealerTanner
		...
		whitelist.add("zaszmedia.com"); // zasz - Frost Dragons Pro, Enchanter
										// Pro, Jars Pro
		whitelist.add("pumyscript.orgfree.com"); // Pumy - Ape Atoll Chinner,
													// PumyDungxFarm,
													// PumyArtisansWorkshop
		whitelist.add("noneevr2.r00t.la"); // noneevr2 - TakeBury
		whitelist.add("testscriptsecurity.host22.com");// Marneus901 - Runite
														// miner
		whitelist.add("massacrescripting.net");// ShizZznit - Aviansie Massacre.
		whitelist.add(".ownagebots.com"); // Ownageful/Aut0r's scripts -
											// OwnageGDK, OwnageBDK, OwnageFDK
	...
	public boolean isCallerScript() {
		return Thread.currentThread().getName().startsWith("Script-")
				|| getCallingClass().startsWith("org.rsbot.script.Script");
	}
